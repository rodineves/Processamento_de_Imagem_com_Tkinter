...
class Application:
    ...
    def frames_da_tela(self):
        self.frame1 = Frame(self.root, bg="lightyellow")
        self.frame1.place(relx=0.02, rely=0.02, relwidth=0.96, relheight=0.5)

        self.frame2 = Frame(self.root, bg="lightblue")
    ...
    def widgets_frame_1(self):
        ...
        self.image_label = Label(self.frame1, image=None, bg="lightyellow")
        self.image_label.place(relx=0.0, rely=0.25, relwidth=0.40, relheight=0.45)
        ...
        self.image_label2 = Label(self.frame1, image=None, bg="lightyellow")
        self.image_label2.place(relx=0.55, rely=0.25, relwidth=0.40, relheight=0.45)
    ...
    def create_widgets_frame_2(self):
        ...
        self.processed_image_label = Label(self.frame2, image=None, bg="lightblue")
        self.processed_image_label.place(relx=0.0, rely=0.19, relwidth=0.30, relheight=0.45)

        # Label para exibir a imagem de diferença
        self.difference_image_label = Label(self.frame2, image=None, bg="lightblue")
        self.difference_image_label.place(relx=0.60, rely=0.19, relwidth=0.10, relheight=0.45)
        
        # Label para exibir o Histograma
        self.hist_image_label = Label(self.frame2, image=None, bg="lightblue")
        self.hist_image_label.place(relx=0.60, rely=0.19, relwidth=0.10, relheight=0.45)
    ...
    def process_image(self):
        if self.image:
            ...
            if self.image2:
                ...
                self.plot_histogram(self.matriz_de_erro)
    ...
    def plot_error_matrix(self, matrix):
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(matrix, cmap='seismic',vmin=-255, vmax=255,  interpolation='nearest', edgecolor='black')
        fig.colorbar(cax)
        ...
        canvas.draw()
        canvas.get_tk_widget().place(relx=0.28, rely=0.12, relwidth=0.35, relheight=0.55)
        
    def plot_histogram(self, matrix):
        matrix = np.array(matrix).flatten()  # Achata a matriz para um array 1D

        # Criar a figura e o eixo para o histograma
        fig, ax = plt.subplots()
        ax.hist(matrix, bins=50, color='blue', edgecolor='black')
        ax.set_title('Histograma da Matriz de Erro')
        ax.set_xlabel('Valor do Erro')
        ...
        canvas = FigureCanvasTkAgg(fig, master=self.frame2)
        canvas.draw()
        canvas.get_tk_widget().place(relx=0.65, rely=0.19, relwidth=0.30, relheight=0.55)
